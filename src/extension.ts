...
export function activate(context: ExtensionContext) {
    ...
    const serverModule = path.join(__dirname, "..", "server", "lib", "index.js");
    ...
    const client = new LanguageClient("mcfunction", "Minecraft Function", serverOptions, clientOptions);

    const disposable = client.start();
    client.onReady().then(() => {
        client.onNotification("mcfunction/shutdown", (reason: string) => {
            window.showWarningMessage(`Minecraft function loader requested to be shutdown with reason ${reason}`);
            client.stop();
            disposable.dispose();
        });
    }).catch(() => {
